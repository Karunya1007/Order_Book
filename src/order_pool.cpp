...
namespace order_book
{
    ...
    OrderPool::OrderPool(std::size_t capacity): nodes(capacity), free_head(0), size(0), capacity(capacity)
    {
    ...
    {
        const char* base_pointer = reinterpret_cast<const char*>(nodes.data());
        const char* order_pointer = reinterpret_cast<const char*>(order);
        std::ptrdiff_t bytes_from_start = order_pointer - base_pointer;
